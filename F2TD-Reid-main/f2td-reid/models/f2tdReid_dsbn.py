...
class DSBN2d_F2TDReid(nn.Module):
    def __init__(self, planes):
        super(DSBN2d_F2TDReid, self).__init__()
        self.num_features = planes
        self.BN_S = nn.BatchNorm2d(planes)
        self.BN_T = nn.BatchNorm2d(planes)
...
class DSBN1d_F2TDReid(nn.Module):
    def __init__(self, planes):
        super(DSBN1d_F2TDReid, self).__init__()
        self.num_features = planes
        self.BN_S = nn.BatchNorm1d(planes)
        self.BN_T = nn.BatchNorm1d(planes)
...
def convert_dsbn_F2TDReid(model, mixup_bn_names, F2TDReid=False):

    for _, (child_name, child) in enumerate(model.named_children()):
        # print(child_name)

        F2TDReid_flag = F2TDReid
        assert(not next(model.parameters()).is_cuda)
        for name in mixup_bn_names:
            if name in child_name:
                F2TDReid_flag = True
        if isinstance(child, nn.BatchNorm2d) and not F2TDReid_flag:
            m = DSBN2d(child.num_features)
            m.BN_S.load_state_dict(child.state_dict())
            m.BN_T.load_state_dict(child.state_dict())
            setattr(model, child_name, m)
        elif isinstance(child, nn.BatchNorm2d) and F2TDReid_flag:
            m = DSBN2d_F2TDReid(child.num_features)
            m.BN_S.load_state_dict(child.state_dict())
            m.BN_T.load_state_dict(child.state_dict())
            m.BN_mix.load_state_dict(child.state_dict())
            setattr(model, child_name, m)
        elif isinstance(child, nn.BatchNorm1d) and not F2TDReid_flag:
            m = DSBN1d(child.num_features)
            m.BN_S.load_state_dict(child.state_dict())
            m.BN_T.load_state_dict(child.state_dict())
            setattr(model, child_name, m)
        elif isinstance(child, nn.BatchNorm1d) and F2TDReid_flag:
            m = DSBN1d_F2TDReid(child.num_features)
            m.BN_S.load_state_dict(child.state_dict())
            m.BN_T.load_state_dict(child.state_dict())
            m.BN_mix.load_state_dict(child.state_dict())
            setattr(model, child_name, m)
        else:
            convert_dsbn_F2TDReid(child, mixup_bn_names, F2TDReid=F2TDReid_flag)


def convert_bn_F2TDReid(model, use_target=True):
    for _, (child_name, child) in enumerate(model.named_children()):
        assert(not next(model.parameters()).is_cuda)
        if isinstance(child, DSBN2d):
            ...
            setattr(model, child_name, m)
        elif isinstance(child, DSBN2d_F2TDReid):
            m = nn.BatchNorm2d(child.num_features)
            if use_target:
                m.load_state_dict(child.BN_T.state_dict())
            else:
            ...
            if use_target:
                ...
                m.load_state_dict(child.BN_S.state_dict())
            setattr(model, child_name, m)
        elif isinstance(child, DSBN1d_F2TDReid):
            m = nn.BatchNorm1d(child.num_features)
            if use_target:
                m.load_state_dict(child.BN_T.state_dict())
            else:
            ...
            convert_bn_F2TDReid(child, use_target=use_target)
