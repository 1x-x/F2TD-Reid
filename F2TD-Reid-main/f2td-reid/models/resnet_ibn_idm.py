...
__all__ = ['ResNetIBN', 'resnet_ibn50a_F2TDReid', 'resnet_ibn101a_F2TDReid']
...
class ResNetIBN(nn.Module):
    ...
    def forward(self, x, stage=0):
        ...
        if stage == 0 and self.training:
            x, attention_lam = self.F2TDReid1(x)
        x = self.layer1(x)
        if stage == 1 and self.training:
            x, attention_lam = self.F2TDReid2(x)
        x = self.layer2(x)
        if stage == 2 and self.training:
            x, attention_lam = self.F2TDReid3(x)
        x = self.layer3(x)
        if stage == 3 and self.training:
            x, attention_lam = self.F2TDReid4(x)
        x = self.layer4(x)
        if stage == 4 and self.training:
            x, attention_lam = self.F2TDReid5(x)
...
def resnet_ibn50a_F2TDReid(**kwargs):
    return ResNetIBN('50a', **kwargs)


def resnet_ibn101a_F2TDReid(**kwargs):
