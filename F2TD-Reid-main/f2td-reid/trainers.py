...
class F2TDReid_Trainer(object):
    def __init__(self, model, xbm, num_classes, margin=None, mu1=1.0, mu2=1.0, mu3=1.0, mu4=1.0, mu5=1.0, HDiuRatio=2,
                 DMomentumUpdate=0.999):
        super(F2TDReid_Trainer, self).__init__()
        self.model = model
        self.xbm = xbm
        self.mu1 = mu1
        self.mu2 = mu2
        ...
        self.HDiuRatio = HDiuRatio
        self.DMomentumUpdate = DMomentumUpdate
        # num_classes是 source_classes+len(dataset_target.train)，train_F2TDReid文件中
        self.num_classes = num_classes
    ...
    def train(self, epoch, data_loader_source, data_loader_target, source_classes, target_classes,
              optimizer, print_freq=50, train_iters=400, use_xbm=True, stage=0):
        ...
        losses_dictionary_learning = AverageMeter()
        losses_aSTLoss = AverageMeter()
        ...
        for i in range(train_iters):
            ...
            targets = targets.view(-1)
            # forward
            # prob, feats, attention_lam, allDL_loss = self._forward(inputs, stage, targets,
            #                                                        self.HDiuRatio,
            #                                                        self.DMomentumUpdate)  # attention_lam: [B, 2]
            prob, feats, attention_lam, allDL_loss, aSTLoss = self._forward(inputs, stage, targets,
                                                                            self.HDiuRatio,
                                                                            self.DMomentumUpdate)  # attention_lam: [B, 2]
            # prob(192,13638),192 = 64*3,3是源域，目标域和中间域
            prob = prob[:, 0:source_classes + target_classes]
            ...
            if use_xbm:
                ...
                loss = (1. - self.mu1) * loss_ce + loss_tri + loss_xbm + \
                       self.mu1 * loss_bridge_prob + self.mu2 * loss_bridge_feat + self.mu3 * loss_diverse + \
                       self.mu4 * allDL_loss + self.mu5 * aSTLoss
            else:
            ...
            losses_dictionary_learning.update(allDL_loss.item())
            losses_aSTLoss.update(aSTLoss.item())
                ...
                if use_xbm:
                    print('Epoch: [{}][{}/{}]\t'
                          ...
                          .format(epoch, i + 1, len(data_loader_target),
                                  ...
                                  losses_dictionary_learning.val, losses_dictionary_learning.avg,
                                  losses_aSTLoss.val, losses_aSTLoss.avg, # 此行为新添加
                                  precisions_s.val, precisions_s.avg,
                                  precisions_t.val, precisions_t.avg
                                  ))
                else:
    ...
    def _parse_data(self, inputs):
        # inputs：图像Tensor，imgpath，pid，cid，indexes
        imgs, _, pids, _, indexes = inputs
        return imgs.cuda(), pids.cuda(), indexes.cuda()

    def _forward(self, inputs, stage, s_t_targets, HDiuRatio, DMomentumUpdate):
        return self.model(inputs, stage=stage, s_t_targets=s_t_targets, HDiuRatio=HDiuRatio,
                          DMomentumUpdate=DMomentumUpdate)
