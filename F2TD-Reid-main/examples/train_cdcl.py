...
from F2TDReid import datasets
from F2TDReid import models
from F2TDReid.models.F2TDReid_dsbn import convert_dsbn_F2TDReid, convert_bn_F2TDReid
from F2TDReid.models.xbm import XBM
from F2TDReid.trainers import Baseline_Trainer, F2TDReid_Trainer
from F2TDReid.evaluators import Evaluator, extract_features
from F2TDReid.utils.data import IterLoader
from F2TDReid.utils.data import transforms as T
from F2TDReid.utils.data.sampler import RandomMultipleGallerySampler
from F2TDReid.utils.data.preprocessor import Preprocessor
from F2TDReid.utils.logging import Logger
from F2TDReid.utils.serialization import load_checkpoint, save_checkpoint, copy_state_dict
from F2TDReid.utils.rerank import compute_jaccard_distance
...
def filter_layers(stage):
    ...
    for i in range(len(layer_names)):
        if i < stage + 1:
            ...
            F2TDReid_bn_names.append(layer_names[i])
    return F2TDReid_bn_names


def create_model(args):
    model = models.create(args.arch, num_features=args.features, norm=False, dropout=args.dropout,
                          num_classes=args.nclass)
    F2TDReid_bn_names = filter_layers(args.stage)
    convert_dsbn_F2TDReid(model, F2TDReid_bn_names, F2TDReid=False)
...
def main_worker(args):
    ...
    # 将前source_classes个类别的权重替换为源域上这些类别的中心，以此来解决源域和目标域的类别分布不一致的问题
    ...
    # Trainer
    trainer = F2TDReid_Trainer(model, xbm, args.nclass, margin=args.margin,
                          mu1=args.mu1, mu2=args.mu2, mu3=args.mu3,mu4=args.mu4,mu5=args.mu5,
                          HDiuRatio=args.HDiuRatio,DMomentumUpdate=args.DMomentumUpdate)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--batch-size', type=int, default=64) #初始bs 是 64
    ...
    parser.add_argument('--mu2', type=float, default=0.1, # 默认0.1
                        help="weight for loss_bridge_feat")
    parser.add_argument('--mu3', type=float, default=1,
                        help="weight for loss_div")
    parser.add_argument('--mu4', type=float, default=1,
                        help="weight for loss_dictionary_learning")
    parser.add_argument('--mu5', type=float, default=10,
                        help="weight for loss_attention_a")

    # model
    parser.add_argument('-a', '--arch', type=str, default='resnet50_F2TDReid',
    ...
    parser.add_argument('--memorySize', type=int, default=8192,
                        help='meomory bank size') # m默认值是8192
    ...
    parser.add_argument('--stage', type=int, default=0,
                        help="insert F2TDReid module after stage 0/1/2/3/4")
    parser.add_argument('--HDiuRatio', type=int, default=3,
                        help="K= H - (H // HDiuRatio)等式中最后K的取值，K表示取字典D的前多少列进行更新，显然K越大越好")
    parser.add_argument('--DMomentumUpdate', type=int, default=0.2,
                        help="用于公共字典动量更新")
    # optimizer
    parser.add_argument('--lr', type=float, default=0.00035,
                        help="learning rate")
    parser.add_argument('--weight-decay', type=float, default=5e-4)
    parser.add_argument('--epochs', type=int, default=50) # 初始值是 50
    parser.add_argument('--iters', type=int, default=400) # 初始值是 400
